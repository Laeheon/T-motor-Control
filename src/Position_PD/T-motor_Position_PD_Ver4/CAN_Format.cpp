...
double CAN_Format::Receive_current_position(){
    CAN_message_t received_frame;
    unsigned long start_time_us = micros();           // Reception waiting starting time (us) 
    const unsigned long timeout_us = 100 * 1000;        // Max waiting time unit changing from 100ms to 100,000us
    int retry_count = 0;                                // Retry times
    const int max_retries = 100;                        // Max Retry Times  
    static int received_count = 0;                      // Resposnse times counter  

    while (retry_count < max_retries) {
        // Continuous Read Attempts During Timeout Period (µs)
        while (micros() - start_time_us < timeout_us) {
            if (can1.read(received_frame)) {
                // Response Reception Time Measurement
                unsigned long receive_time_us = micros();
                unsigned long response_time_us = receive_time_us - start_time_us;
                received_count++;
                if (received_count % 50 == 0) {
                    Serial.print("Response time (us): ");
                    Serial.println(response_time_us);
                }
                
                Serial.println("Position PD Control - Response received!");

                // Value interpretaton according to the manual 
                int motor_position = (received_frame.buf[1] << 8) | received_frame.buf[2];
                double scaled_position = Receiving_scaling_value(motor_position, Peak_position, -Peak_position, 16);
                ...
                if (motor_error_flag == 1) {
                    ...
                    Serial.println("No error in CAN message.");
                    // Saving value to class member variable 
                    received_position_position_PD = scaled_position;
                    received_speed_position_PD = scaled_speed;
                    return scaled_position;
                }
            }
        }
        // Retry when timeout occured 
        Serial.println("Position PD Control - Response timeout. Retrying...");
        retry_count++;
        start_time_us = micros();  // Restarting time initialization (us)
    }

    // Return default value when reception failed after max retry 
...
void CAN_Format::Position_Control_PD(uint8_t driver_id, double Current_value) {
    ...
    frame.buf[4] = Kp_hex & 0xFF;                   // Kp_hex Low 8 bits
    frame.buf[5] = (Kd_hex >> 4) & 0xFF;            // Kd_hex High 8 bits
    frame.buf[6] = ((Kd_hex & 0xF) << 4) | ((cur_hex >> 8) & 0xF); // Kd_hex Low 4 bits + cur_hex High 4 bits
    frame.buf[7] = cur_hex & 0xFF;                  // cur_hex Low 8 bits
