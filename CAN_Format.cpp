...
void CAN_Format::Position_Control(uint8_t driver_id, double Motor_position, double Motor_speed, int Kp_value, int Kd_value){
    ...
    Serial.println("CAN frame 0:");
    Serial.println((pos_hex >> 8) & 0xFF);
    Serial.println("CAN frame 1:");
    Serial.println((pos_hex & 0xFF));
    Serial.println("Speed hex value:");
    Serial.println(spd_hex);
...
void CAN_Format::Torque_Control(uint8_t driver_id, double Current_value) 
{
...
double CAN_Format::Receive_current_torque(){
   CAN_message_t received_frame;
    ...
    Serial.println("Warning: No CAN message received, returning default value.");
    return 0.0; 
}

double CAN_Format::Torque_Control_PD(uint8_t driver_id, double Current_value) {
    CAN_message_t frame;
    frame.id = driver_id;  
    frame.len = 8;        
    ...
    while (retry_count < max_retries) {
        while (millis() - start_time < timeout) {
            if (can1.read(received_frame)) {
                Serial.println("Torque PD Control - Response received!");
                ...
                if (motor_error_flag == 1) {
                    ...
                    Serial.println("No error in CAN message.");
                    return scaled_current; // 수신된 값 반환
                }
            }
        }
